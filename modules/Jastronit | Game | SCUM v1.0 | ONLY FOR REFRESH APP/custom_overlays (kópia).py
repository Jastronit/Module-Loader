...
def create_widget(BaseClass, module_name):
    class CustomOverlaysWidget(BaseClass):
        ...
        def toggle_selected_overlay(self):
            ...
            # názov overlayu (odstránime prípadný "✅"/"❌")
            fname = selected.text().split()[0]
            ...
            if full_name in mgr.overlays:
                ...
                # nastav vnútorný stav aj viditeľnosť
            ...
            try:
                ...
                for cname, params in data.items():
                    ...
                    if name in mgr.overlays:
                        ...
                        # overlay ešte nebol načítaný (napr. nový zápis v json)
                        if params.get("user_visible", True):
                            item_text += " ✅"
                        else:
                            item_text += " ❌"
                    self.overlay_list.addItem(item_text)
