...
def create_widget(BaseClass, module_name):
    class CustomOverlaysWidget(BaseClass):
        def __init__(self):
            ...
            # --- Pridanie tlačidiel ---
            btn_create = QPushButton("Create Custom Overlay")
            btn_create.clicked.connect(self.create_custom_overlay)
            ...
            self.overlay_list.itemClicked.connect(self.select_overlay)
        ...
        def create_custom_overlay(self):
        ...
        def select_overlay(self, item):
            self.selected_overlay = item.text()
